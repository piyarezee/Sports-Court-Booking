...
export default function AdminBookings() {
  ...
  // Glass card classes based on status
  const getGlassClass = (status) => {
    if (status === 'approved') return 'bg-emerald-400/10 backdrop-blur-xl border border-emerald-400/30 shadow-emerald-200/50'
    if (status === 'rejected') return 'bg-red-400/10 backdrop-blur-xl border border-red-400/30 shadow-red-200/50'
    return 'bg-amber-400/10 backdrop-blur-xl border border-amber-400/30 shadow-amber-200/50' // pending
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      {/* Background Blobs for Glass Effect */}
      <div className="absolute top-0 left-0 w-96 h-96 bg-purple-600/20 rounded-full filter blur-[120px]"></div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 hidden sm:block">Review and manage customer bookings</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        {error && <div className="bg-red-500/10 text-red-400 backdrop-blur-md border border-red-500/30 px-4 py-3 rounded-xl mb-4 text-sm font-medium">{error}</div>}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {bookings.map(b => (
              <div key={b.id} className={`rounded-2xl shadow-lg transition-all duration-300 hover:scale-[1.01] overflow-hidden ${getGlassClass(b.status)}`}>
                <div className="p-5 sm:p-6 bg-slate-900/20">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <a 
                          href={`https://wa.me/${formatWhatsAppNumber(b.mobile)}`} 
                          target="_blank" 
                          rel="noreferrer" 
                          className="mt-2 inline-flex items-center gap-2 bg-green-500/20 hover:bg-green-500/40 text-green-300 px-3 py-1.5 rounded-full font-semibold text-sm border border-green-400/40 transition-colors w-fit shadow-sm"
                        >
                  ...
                  <div className="mt-5 pt-5 border-t border-white/10 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-white">{b.slotStart} - {b.slotEnd}</p>
                    </div>
                    <div>
                      <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Amount</p>
                      <p className="font-semibold text-white">Rs. {b.amount}</p>
                    </div>
                  </div>
                  ...
                  {b.status === 'pending' && (
                    <div className="mt-5 flex flex-col sm:flex-row gap-3">
                      <button
                        ...
                        className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-3 rounded-xl text-sm transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                      ...
                      <button
                        ...
                        disabled={actionLoading === b.id}
                        className="flex-1 sm:flex-initial bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-red-500/30 hover:border-red-400/40 font-semibold py-3 px-6 rounded-xl text-sm transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
